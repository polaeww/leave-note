...
<section class="wrapper">
    <br>
    <h3><i class="fa fa-angle-right"></i> แก้ไขใบลา</h3>
    <div class="row mt">
        <div class="col-lg-12">
            <div class="form-panel">
                <br>
                <form class="form-horizontal style-form" action="{{route('leave.save',['leave' => $leave])}}" method="post">
                    {{csrf_field()}}
                    <div class="form-group">
                        <div class="col-sm-1"></div>
                        <label class="col-sm-1 col-sm-1 control-label">ประเภทใบลา:</label>
                        <div class="col-sm-4">
                            @foreach($leave_length_types as $leave_length_type)
                            <input style="margin-left: 20px" type="radio" name="leave_length_type_id" value="{{$leave_length_type->id}}" @if($leave_length_type->id
                            == $leave->leave_length_type_id) {{"checked"}} @endif> {{$leave_length_type->name}} @endforeach()
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-1"></div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2">
                            <input type="date" value="{{$leave->date_start}}" class="form-control" name="date_start">
                        </div>
                        <!--ถึงเวลา-->
                        <label class="col-sm-1 col-sm-2 control-label">ถึงวันที่:</label>
                        <div class="col-sm-2">
                            <input type="date" value="{{$leave->date_end}}" class="form-control" name="date_end">
                        </div>
                    </div>
                    <!--เวลาที่ลา-->
                    <div class="form-group">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>
                                                    ...
                                                    <input style="margin-left: 20px" type="radio" name="leave_type_id" value="{{$leave_type->id}}" @if($leave_type->id
                                                    == $leave->leave_type_id) {{"checked"}}@endif()> {{$leave_type->name}}
                                                    @endforeach()
                                                </td>
                                                <td data-title="เหตุผลการลา"> <input type="text" class="form-control" name="reason" value="{{$leave->reason}}"></td>
                                                <td data-title="หมายเหตุ"> <input type="text" class="form-control" name="note" value="{{$leave->note}}">                                                    </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <div class="row mt">
